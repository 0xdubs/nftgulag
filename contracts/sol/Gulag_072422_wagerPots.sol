...
contract Gulag {
    ...
    function redeemEarnings(uint _battleNumber) public payable {
        ...
        require ((block.timestamp-indBattle.battleTiming.startTime)>60,"Wager period hasn't finished yet");
        require ((block.timestamp-indBattle.battleTiming.startTime)>(120),"voting period hasn't finished yet");
        if (indBattle.creatorVoteCount==indBattle.challengerVoteCount) {
            ...
            if (wagerByAddressForEachBattleByFighter[msg.sender][_battleNumber][0]>0) {
                ...
                mult1 = (wagerByAddressForEachBattleByFighter[msg.sender][_battleNumber][0]-wagerByAddressForEachBattleByFighter[indBattle.battleCreator.battleCreator][_battleNumber][0]);
                mult1 = mult1.mul((mappedTotalWagerAmountByBattleForChallenger[_battleNumber]-wagerByAddressForEachBattleByFighter[indBattle.battleChallenger.challenger][_battleNumber][1]));
                ...
                mult1 = (wagerByAddressForEachBattleByFighter[msg.sender][_battleNumber][1]-wagerByAddressForEachBattleByFighter[indBattle.battleChallenger.challenger][_battleNumber][1]);
                mult1 = mult1.mul((mappedTotalWagerAmountByBattleForCreator[_battleNumber]-wagerByAddressForEachBattleByFighter[indBattle.battleCreator.battleCreator][_battleNumber][0]));
    ...
    function voteForNFT(uint battleNumber, uint voteGetter) public {
        ...
        require((block.timestamp-indBattle.battleTiming.startTime)>60,"Wager period hasn't finished yet - you can't vote!");
        require((block.timestamp-indBattle.battleTiming.startTime)<(120),"Voting period is over");
    ...
    function wagerOnNFT(uint battleNumber, uint wagerGetter) public payable {
        ...
        require(indBattle.battleTiming.battleStarted==true,"Battle hasn't even started yet");
        require(indBattle.battleTiming.startTime>0,"sorry - this battle hasn't started yet");
        require((block.timestamp-indBattle.battleTiming.startTime)<60,"Sorry - the wager period for this battle is over");

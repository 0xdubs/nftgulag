...
const CompletedGulags = () => {
  ...
  const redeemWinnings = async (battleStartTimeMs: any,battleNumber: any) => {
    if ((DateTime.fromJSDate(new Date(battleStartTimeMs*1000))).plus({days:28})<DateTime.now().toUTC()) {
      toast?.pushError(`Redemption Error: Can't redeem earnings yet - voting period is not over`, 10000)
      return
    }
    ...
    if (contract) {
      ...
      .redeemEarnings(
       battleNumber)
        .then(() => {
          toast?.pushSuccess("Successfully redeemed earnings",10000)
        })
        .catch(error => {
          toast?.pushError(`Redemption Error: ${error.error.message}`, 10000)
          console.error(error);
        });
    }
  }
          ...
          for (let i = 0; i<totalBattles.toNumber() ; i++) {
            ...
            .battles(
              ...
              .then(async (response) =>   {
                ...
                //@ts-ignore
                if (response.battleChallenger.challenger!=="0x0000000000000000000000000000000000000000" && DateTime.fromJSDate(new Date(response.battleTiming.startTime*1000)).plus({days:10})<DateTime.now()) {
                  gulagsArray.push(
  ...
  return (
    ...
    gulags.length===0 && (
      ...
      gulags.length>0 &&
        ...
        <div style={gulag.battleCreatorVoteCount>=gulag.battleChallengerVoteCount  ? {border:"9px solid #4BB543",borderRadius:"20px"}:{}}>
          <NFT
        ...
        flexWrap: "wrap"}}>Total Wagered: {Math.round(gulag.battleTotalWagered*100)/100} ETH </div>
              <div style={{display: "flex",
        justifyContent: "space-evenly",
        flexWrap: "wrap"}}>Creator Votes: {gulag.battleCreatorVoteCount.toNumber()} </div>
              <div style={{display: "flex",
        justifyContent: "space-evenly",
      ...
      <BsFillInfoCircleFill size={24} />
            ...
            <div>
            <button className="interact-button-redeem" onClick={() => redeemWinnings(gulag.battleStartTime,gulag.battleNumber)}>Redeem</button>
            </div>
            </div>
           <div style={gulag.battleCreatorVoteCount<=gulag.battleChallengerVoteCount  ? {border:"9px solid #4BB543",borderRadius:"20px"}:{}}>
          <NFT
...
export default CompletedGulags;

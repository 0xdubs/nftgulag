...
import NFTSkeleton from "../NFTStylized/NFTSkeleton";
...
const MyGulags = () => {
  ...
  const [gulags,setGulags]: [any[],any] = useState([])
          ...
          for (let i = 0; i<totalBattles.toNumber() ; i++) {
            ...
            .battles(
              ...
              .then(async (response) =>   {
                ...
                //@ts-ignore
                if (response.battleChallenger.challenger===account || response.battleCreator.battleCreator===account) {
                  gulagsArray.push(
                    ...
                    battleCreatorVoteCount: response.creatorVoteCount,
                    battleChallengerVoteCount: response.challengerVoteCount
                  })
    ...
    gulags.length===0 && (
      ...
      gulags.length>0 &&
        ...
        <div>
            ...
            <div style={{width:"70%"}} className="items">
        ...
        <div style={gulag.challengerNftAddress==="0x0000000000000000000000000000000000000000" ? {} :DateTime.fromJSDate(new Date(gulag.battleStartTime*1000)).plus({days:7})>DateTime.now() ?{}: gulag.battleCreatorVoteCount>=gulag.battleChallengerVoteCount  ? {border:"9px solid #4BB543",borderRadius:"20px"}:{}}>
          {gulag.creatorNftAddress==="0x0000000000000000000000000000000000000000" ? (
            <NFTSkeleton />
          ): <NFT
          ...
          tokenId={gulag.creatorNftTokenId} /> }
            ...
            <Tooltip style={{alignItems: "bottom",color: "indigo",background:"indigo"}} content={
              (<>
              <div>
        ...
        flexWrap: "wrap"}}>Total Wagered: {Math.round(gulag.battleTotalWagered*100)/100} ETH </div>
              {gulag.battleStartTime.toNumber()!==0 &&DateTime.fromJSDate(new Date(gulag.battleStartTime*1000)).plus({days:14})<DateTime.now() && (
                <div style={{display: "flex",
                justifyContent: "space-evenly",
                flexWrap: "wrap"}}>
            {`Creator Votes: ${gulag.battleCreatorVoteCount.toNumber()}`}
           </div>)
            }
           ...
           {gulag.battleStartTime.toNumber()!==0 &&DateTime.fromJSDate(new Date(gulag.battleStartTime*1000)).plus({days:14})<DateTime.now() && (
              <div style={{display: "flex",
        justifyContent: "space-evenly",
        flexWrap: "wrap"}}>{`Challenger Votes: ${gulag.battleChallengerVoteCount.toNumber()}`} </div>)}
              {gulag.battleStartTime.toNumber()!==0 &&DateTime.fromJSDate(new Date(gulag.battleStartTime*1000)).plus({days:14})<DateTime.now() && (
              <div style={{display: "flex",
        justifyContent: "space-evenly",
        flexWrap: "wrap"}}>Winner: {gulag.battleCreatorVoteCount.toNumber()>gulag.battleChallengerVoteCount.toNumber() ? "Creator": gulag.battleCreatorVoteCount.toNumber()<gulag.battleChallengerVoteCount.toNumber() ? "Challenger": gulag.battleCreatorVoteCount.toNumber()===gulag.battleChallengerVoteCount.toNumber() ? "Tie": "Unknown result"} </div>)}
              </>)}>
      ...
      <BsFillInfoCircleFill size={24} />
           ...
           <div style={gulag.challengerNftAddress==="0x0000000000000000000000000000000000000000" ? {} :DateTime.fromJSDate(new Date(gulag.battleStartTime*1000)).plus({days:7})>DateTime.now() ?{}:gulag.battleCreatorVoteCount<=gulag.battleChallengerVoteCount  ? {border:"9px solid #4BB543",borderRadius:"20px"}:{}}>
           {gulag.challengerNftAddress==="0x0000000000000000000000000000000000000000" ? (
            <NFTSkeleton />
          ): 
          <NFT

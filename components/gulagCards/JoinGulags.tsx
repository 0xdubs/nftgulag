...
// import { Chain, Loading, NFT} from "web3uikit"
import { NFT } from "../NFTStylized";
import { Loading,Chain } from "web3uikit";
import { CardGroup,Card
  } from 'reactstrap';
...
const JoinGulags = () => {
  ...
  const [nftChain, setNftChain] = useState("" as Chain)
  ...
  const getNFTOwner = async (nftChain:any,nftAddress:any,nftTokenId:any,proposedOwnerAddress:any) => {
      let baseURL;
      switch (nftChain) {
        case 'eth':
            baseURL=`https://eth-mainnet.alchemyapi.io/nft/v2/${process.env.NEXT_PUBLIC_ALCHEMY_API_KEY}/getOwnersForToken`;
            break;
        case 'polygon':
            baseURL=`https://polygon-mainnet.alchemyapi.io/nft/v2/${process.env.NEXT_PUGLIC_ALCHEMY_API_KEY}/getOwnersForToken`;
            break;
        default:
            break;
        }
      ...
      const owners = await fetch(fetchURL, {method:"GET"})
      .then(response => response.json())
      .catch(error => console.log('error', error));
      if (typeof(owners.owners)==="object") {
        const ownerArray = owners.owners;
        return ownerArray.includes(proposedOwnerAddress.toLowerCase())
      }
      return false    
  }
  
  const joinGulag = async (transactionSenderAddress: string | null | undefined,battleNumber: BigNumberish,nftChain: string,nftAddress: string,nftTokenId: BigNumberish, ethForCall: string) => {
    ...
    const isOwner = await getNFTOwner(nftChain,nftAddress,nftTokenId,transactionSenderAddress)
    if (!isOwner) {
        toast?.pushError("Sorry - we couldn't verify you are the owner of this NFT",10000)
    }
    if (contract && isOwner) {
      await contract
      .joinGulag(
       battleNumber, nftChain,nftAddress,nftTokenId,
      {gasLimit:4000000,value:payableValue})
        .then(response => {
            toast?.pushSuccess("Successfully joined gulag!",10000)
          console.log("response", response);
        })
        .catch(error => {
            toast?.pushError(`Error joining gulag: ${error}`,10000)
          console.error(error);
        });
    }
  }
              ...
              .then(async (response:any) =>   {
                ...
                if (response.battleChallenger.challenger==="0x0000000000000000000000000000000000000000") {
                  gulagsArray.push(
                    ...
                    creatorNftTokenId: response.battleCreator.battleCreatorNFTTokenId,
                    ...
                    challengerNftTokenId: response.battleChallenger.challengerNFTTokenId,
  ...
  return (
    ...
    gulags.length===0 && (
      ...
      gulags.length>0 &&
        ...
        <div>
            ...
            <div className="items w-2/3 sm:w-1/3">
              {gulags.slice(firstContentIndex,lastContentIndex).map((gulag:any,index: any)=> (
                
                <div className="card-group w-70vw" key={index}>
        ...
        <div className="text-center justify-items-center" style={{border:"150%"}}>
          <div id='nft-info' className="grid justify-center flex-row align-middle text-center">
          ...
          <NFT
            ...
            tokenId={gulag.creatorNftTokenId} /> 
                        ...
                        <div className="form">
            <Card className="flex flex-row text-center content-center" style={{
        display: "flex",
        justifyContent: "space-evenly",
        flexWrap: "wrap",
        marginBottom: "40px"
      }}>
          <button className="interact-button-wager" onClick={() => setOpen(true)}>Join</button>
      ...
      <Dialog as="div" className="relative z-10" initialFocus={cancelButtonRef} onClose={setOpen}>
        ...
        <div className="fixed z-10 inset-0 overflow-y-auto">
          <div className="flex items-center justify-center min-h-full p-4 text-center lg:p-5">
            ...
            >
              <Dialog.Panel className="md m-5 relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all lg:my-8 max-w-lg w-full">
                <div className="bg-white flex justify-center ml-5 mt-2 px-4 pt-5 pb-4 lg:p-6 lg:pb-4">
                  <div className="flex mt-2 lg:items-start">
                    <div className="mx-auto absolute z-0 flex-shrink-0 flex mt-3 items-center justify-center h-12 w-12 rounded-full bg-green-100 lg:mx-0 lg:h-10 lg:w-10">
                      <CurrencyDollarIcon className="h-6 w-6 text-green-600" aria-hidden="true" />
                    </div>
                    <div className="interact-button-vote mt-3 text-center sm:mt-0 sm:ml-4 sm:text-center">
                      <Dialog.Title as="h3" className="mt-2 interact-button-vote text-lg leading-6 font-medium text-gray-900">
                        Join Gulag # {gulag.battleNumber+1}
                      </Dialog.Title>
                      <div className="mt-2">
                      <div className="flex flex-row justify-center form-row mr-3 ml-4" style={{display: "flex",
        justifyContent: "space-evenly",
        flexWrap: "wrap",
        marginBottom: "20px",
        marginTop: "20px"}}>
        ...
        <Select  className="nft-center" options={options} onChange={(event:any) => setNftChain(event.value as Chain)} placeholder="Select an option" name="callDataNftChain"/>
        {/* <input type="" onChange={(event) => setCallDataNftChain(event.target.value as Chain)}></input> */}
        ...
        <input placeholder="Ex. 0x12345678987654321" type="string" onChange={(event) => setNftAddress(event.target.value)}></input>
        <span>NFT Token ID</span>
        <input placeholder="Ex. 121" type="string" onChange={(event) => setNftTokenId(event.target.value)}></input>
        {/* <span className="hint-text">*An additional Ξ0.01 is required for the bot to execute the future transaction. The remainder will be sent back to you if there is any.</span> */}
      </div>
                      </div>
                    </div>
                ...
                <div className="bg-gray-300 px-4 border-8 border-gray-300 mt-3 mb-3 sm:px-6 justify-evenly flex flex-row-reverse">
                  <button
                    ...
                    onClick={async() => {
                      await joinGulag(account,gulag.battleNumber,nftChain,nftAddress,nftTokenId, ethAmount)
                      setOpen(false)
                    }
    ...
    <div className="interact-button-wager-no-border">
            <Tooltip style={{alignItems: "bottom"}} content={
              (<>
              <div>
                {gulag.battleStartTime.toNumber()!==0 ? `Wager Period: ${(DateTime.fromJSDate(new Date(gulag.battleStartTime*1000))).toFormat("LLL dd yyyy")}-${(DateTime.fromJSDate(new Date(gulag.battleStartTime*1000))).plus({days:14}).toFormat("LLL dd yyyy")}`:"Wager period: Hasn't started"}
              </div>
              <div>
                {gulag.battleStartTime.toNumber()!==0 ? `Voting Period: ${(DateTime.fromJSDate(new Date(gulag.battleStartTime*1000))).plus({days:14}).toFormat("LLL dd yyyy")}-${(DateTime.fromJSDate(new Date(gulag.battleStartTime*1000))).plus({days:28}).toFormat("LLL dd yyyy")}`: "Voting period: Hasn't started"}
              </div>
  ...
  <style jsx>{`
        ...
        * {
        ...
        .form .interact-button-wager-no-border {
          font-size: 20px;
          color: black;
          font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;
          padding: 8px 20px;
